...
import { generateToken, verificationByStudent } from "../middleware/authMiddleware.js";

const RegisterStudent = AsyncErrorHandler(async (req, res, next) => {
  const { name, email, password, registrationNo, branch, verified, status } = req.body ;
   if (!name) {
    return next(new Errorhandler("Please Enter your Name", 400));
  }
  if (!email) {
    return next(new Errorhandler("Please Enter your college domain email address", 400));
  }
  if (!password) {
  ...
  // if(!status){
    ...
    await verificationByStudent(req, res, email, token) ;
    res.status(201).json({message: "Verification email sent!"}) ; 
  // }
  // else return next(new Errorhandler("A verification link is already sent !", 400)) ;
